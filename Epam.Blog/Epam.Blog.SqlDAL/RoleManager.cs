    ...
    public class RoleManager : RoleProvider
    {
        ...
        public override bool IsUserInRole(string userlogin, string roleName)
        {
            ...
            using (var _connection = new SqlConnection(_connectionString))
            {
                ...
                var query3 = "SELECT ID FROM dbo.UsersRoles" + $"WHERE UserID=({query1}) AND RoleID=({query2})";
        ...
        public override string[] GetRolesForUser(string userlogin)
        {
            ...
            using (var _connection = new SqlConnection(_connectionString))
            {
                ...
                var query = $"SELECT Name FROM dbo.Roles WHERE Id IN (SELECT RoleID FROM dbo.UsersRoles WHERE UserID={user.Id})";
        ...
        public void AddUserToRole(string userLogin, string roleName)
        {
            ...
            using (var _connection = new SqlConnection(_connectionString))
            {
                ...
                var query = $"INSERT INTO dbo.UsersRoles(UserID, RoleID) SELECT {userId}, ID FROM dbo.Roles WHERE Name='{roleName}'";
                var command = new SqlCommand(query, _connection);
